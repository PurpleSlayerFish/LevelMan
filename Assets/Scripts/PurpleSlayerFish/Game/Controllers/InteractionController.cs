...
namespace PurpleSlayerFish.Game.Controllers.Impls
{
    public class InteractionController
    {
        ...
        public AInteraction CheckIntersections(Vector3 position,float playerOffset)
        {
            for (int i = 0; i < Interactions.Count; i++)
                if (_vectorUtils.InDistance(Interactions[i].transform.position - position, Interactions[i].Offset + playerOffset))
